...
class PlotPage(tk.Frame):
    ...
    def create_multiple_color_inputs(self, label_text, default_color, row_start, num_sources, entries_list):
        ...
        if not entries_list:
            tk.Label(self, text=label_text + ":").grid(row=row_start, column=0, pady=5, sticky='e')
        for i in range(num_sources):
            # Skip creating the default entry if it already exists
            if i == 0 and entries_list:
                continue
            color_entry = tk.Entry(self, width=10)
    ...
    def create_single_color_inputs(self, row_start):
        for param, default_color in self.plot_params_defaults.items():
            tk.Label(self, text=f"{param.replace('_', ' ')}:").grid(row=row_start, column=0, pady=5, sticky='e')
            color_entry = tk.Entry(self, width=10)
    ...
    def __init__(self, parent, controller):
        ...
        self.title_label = tk.Label(self, text="Plots Parameters", font=self.subtitle_font, fg="black")
        ...
        save_button = tk.Button(self, text="Save and Submit", command=controller.save_all_data)
        save_button.grid(row=50, column=3)
        
        next_button = tk.Button(self, text="Next", command=lambda: controller.show_frame("RunPage"))
        next_button.grid(row=51, column=1, sticky='w')
        
        # Button to go back to StartPage
        back_button = tk.Button(self, text="Back", command=controller.show_previous_page_from_plot)
