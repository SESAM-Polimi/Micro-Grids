...
def Model_Resolution(model, datapath="Inputs/Parameters.dat",options_string="mipgap=0.05",
    ...
    if (Battery_Degradation == 1 and MILP_Formulation == 0):
        print('###########################################################')
        print('INPUT NOT VALID: if MILP_Formulation NOT activated (= 0), Battery_Degradation must be NOT activated (= 0) too!')
...
#%% Electricity generation system constraints 
    ...
    if Model_Components == 0 or Model_Components == 2:
        model.RESInst = Constraint(model.years_steps,
                                           model.renewable_sources, 
                                           model.generator_types,
                                           rule=C.RES_inst)
    ...
    if Model_Components == 0 or Model_Components == 1:
        ...
        if MILP_Formulation:
            ...
            if Battery_Degradation == 1:
               model.BatteryDegradation  = Constraint(model.scenarios,
                                                       model.years_steps,
                                                       model.periods,
                                                       rule=C.Battery_Bank_Degradation) 
...
##############################################################################################################################################################
        ...
        if Solver:
           ...
           if MILP_Formulation:
              opt.set_options('Method=3 BarHomogeneous=1 Crossover=1 MIPfocus=1 BarConvTol=1e-3 OptimalityTol=1e-3 FeasibilityTol=1e-4 TimeLimit=1000')
           else:
