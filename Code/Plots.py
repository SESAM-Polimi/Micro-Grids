...
#%%
def Battery_SOH(instance,Time_Series,PlotScenario,PlotResolution,PlotFormat):
    ...
    #%% Series preparation
    pDay = 24/instance.Delta_Time()                       # Periods in a day
    # foo = pd.date_range(start=instance.StartDate(),periods=1,freq='1month')      # Assign the start date of the graphic to a dumb variable
    Start_Plot = 0
    PlotTime = Y * 365 
    End_Plot     = Start_Plot + PlotTime*pDay # Create the end of the plot position inside the time_series
    Series.index = range(1,(len(Series)+1))
    Series       = Series[Start_Plot:int(End_Plot)] # Extract the data between the start and end position from the Time_Series
    Series.index = pd.date_range(start=0, periods=PlotTime*pDay, freq=('1h')) 
   
    y_BESS_SOH_1    = Series.loc[:,idx[:,'Battery Bank Capacity',:,:]].values.flatten()/(BESSUnits[PlotStep]*BESSNominalCapacitymilp)*100
    
    y_BESS_SOH = []
    for i in range(0,365):
        if i%30 == 0:
            y_BESS_SOH.append(y_BESS_SOH_1[i])
    
    y_Stacked_pos = []    
    y_Stacked_pos += [y_BESS_SOH]  
    
    # y_Demand   = Series.loc[:,idx[:,'Electric Demand',:,:]].values.flatten()/1e3
    x_Plot = np.arange(len(y_BESS_SOH))
    ...
    Colors_pos += ['#'+BESS_Color]  
    Labels_pos = []
    Labels_pos += ['Battery SOH']  
    
    fig,ax1 = plt.subplots(nrows=1,ncols=1,figsize = (12,8))

    ax1.stackplot(x_Plot, y_Stacked_pos, linewidth=4, labels=Labels_pos, colors=Colors_pos, zorder=2)
    ax1.plot(x_Plot, np.zeros((len(x_Plot))), color='black', label='_nolegend_', zorder=2)
  
    ax1.set_xlabel('Time [Hours]', fontsize=fontaxis)
    ax1.set_ylabel('Battery state of health [%]', fontsize=fontaxis)
    ...
    # if (BESSUnits[PlotStep]*BESSNominalCapacitymilp) >= 1:
    #     ax2=ax1.twinx()
    #     ax2.plot(x_Plot, y_BESS_SOC, '--', color='black', label='Battery SOC', zorder=2)
    #     ax2.set_ylabel('Battery state of charge [%]', fontsize=fontaxis)
    ...
    fig.savefig('Results/Plots/BatterySOH.'+PlotFormat, dpi=PlotResolution, bbox_inches='tight')