...
def ResultsSummary(instance, Optimization_Goal, TimeSeries):
    ...
    with pd.ExcelWriter('Results/Results_Summary.xlsx') as writer:
...
def TimeSeries(instance):
    ...
    "Battery Degradation Effect"
    if instance.MILP_Formulation.value == 1 and instance.Battery_Degradation.value == 1:
        BESS_updated_Capacity           = instance.Battery_Bank_Energy.get_values()
    ...
    for s in range(1,S+1):
        TimeSeries[s] = {}
        with pd.ExcelWriter('Results/Time_Series_SC_%d.xlsx' % (s)) as writer:
         for y in range(1,Y+1):
            ...
            if instance.Model_Components.value == 0 or instance.Model_Components.value == 1:
                "Battery Degradation Effect"
                if instance.MILP_Formulation.value == 1 and instance.Battery_Degradation.value == 1:
                    SOH_Capacity     = pd.DataFrame([BESS_updated_Capacity[(s,y,t)] for t in range(1,P+1)])
                    TimeSeries[s][y] = pd.concat([TimeSeries[s][y], SOH_Capacity], axis=1)
                    scenario_header  += ['Scenario ' + str(s)]
                    flow_header      += ['Battery Bank Capacity']
                    component_header += ['']
                    unit_header      += ['Wh']
